...
<input class="input inp" type="text"placeholder="Ban Number" name="ban">         
<button name="submit" class="btn btn-danger">Ban
        </div>        
</form>';
}
if(isset($_POST['submit'])){
...
if (file_exists(''.$session.'/bannumber/'.$ban.'.json')){
echo '<script>alert("This Number Already Banned")</script>';
echo"<meta http-equiv='refresh' content='0;url=bannumber.php'>";
return;
  }
...
if( is_dir("$session/bannumber") === false)
{mkdir("$session/bannumber");}

$filename=''.$session.'/counte/ban.json'; 
if(!file_exists($filename)){ $id= 0; } else $id = file_get_contents ($filename); $id++; 
file_put_contents(''.$session.'/counte/ban.json',$id);

file_put_contents(''.$session.'/bannumber/'.$id.'.json',$ban);

file_put_contents(''.$session.'/bannumber/'.$ban.'.json',"0");
